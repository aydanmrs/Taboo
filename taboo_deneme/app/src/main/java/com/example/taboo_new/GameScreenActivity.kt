...
class GameScreenActivity : AppCompatActivity() {
    ...
    private fun showShortCustomDialog() {
        ...
        val customDialog = Dialog(this).apply {
            ...
            setCancelable(false) // Arka plan tıklanamaz
        }
        ...
        dialogBinding.restart.setOnClickListener {
            // Restart as a new Team 1 game
            val intent = Intent(this, GameScreenActivity::class.java).apply {
                addGameType(GameType.TEAM1) // Set game type to TEAM1
                putExtra("time", intent.getIntExtra("time", 0)) // Use initial time
                putExtra("passes", initialPasses) // Reset passes to initial
                ...
                putExtra(ResultActivity.RIGHT_COUNT_T1, 0) // Reset Team 1's score
        ...
        customCountdownTimer?.onFinish = {
            ...
            try {
                if (timeUpSound != null && ::timeUpSound.isInitialized)
                    timeUpSound.start() // Saniye bitim ses efekti çal
    ...
    private fun navigateToStatsActivity() {
        when (gameType) {
            ...
            GameType.TEAM1 -> {
                // Show dialog to navigate to second team
                val dialogView = layoutInflater.inflate(R.layout.alert_dialog, null)
                val dialogBuilder = AlertDialog.Builder(this)
                    .setView(dialogView)
                    .setCancelable(false) // Diğer alanlara tıklanamaz
                    .setOnCancelListener {
                        // Burada kullanıcı diyalog kutusunun dışına tıklarsa ne olacağını belirtebilirsiniz
                    }

                val alertDialog = dialogBuilder.create()

                // Bu satır arka plan tıklamasını engeller
